...
public:
    ...
    size_t num_features;
    float strong_threshold;
    static bool selectScatteredFeatures(cv::Mat src, cv::Mat mask, cv::Mat quantized_angles, cv::Mat angles, const std::vector<Candidate> &candidates,
                                                                            std::vector<Feature> &features,
...
public:
    ...
    void produce_infos(){
        ...
        else if (angle_range.size() == 4 && scale_range.size() == 1) {
            float scale = scale_range[0];
            for (float angle = angle_range[0]; angle <= angle_range[1] + eps; angle += angle_step) {
                infos.emplace_back(angle, scale);
            }
            for (float angle = angle_range[2]; angle <= angle_range[3] + eps; angle += angle_step) {
                infos.emplace_back(angle, scale);
            }
        }
    }
