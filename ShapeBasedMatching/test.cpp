...
static std::string prefix = "C:/Users/lifuw/Desktop/Projects/template_matching/CPPConsole 2020230626/CPPConsole/TestImages/";
...
void scale_test(string mode = "test") {
    ...
    if (mode == "train") {
        Mat img;
        cv::cvtColor(imread(prefix + "train.bmp"), img, COLOR_BGR2GRAY);
        assert(!img.empty() && "check your img path");
        shape_based_matching::shapeInfo_producer shapes(img);

        shapes.scale_range = { 0.5f, 1.5f };
        ...
        // save templates
        detector.writeClasses(prefix + "case1/test_templ.yaml");
        ...
        detector.readClasses(ids, prefix + "case1/test_templ.yaml");

        Mat test_img = imread(prefix + "test.bmp"); 
...
void angle_test(string mode = "test", bool use_rot = true) {
    ...
    if (mode != "test") {
        Mat img;
        assert(!img.empty() && "check your img path");
        cv::cvtColor(imread(prefix + "template.jpg"), img, COLOR_BGR2GRAY);

        //Rect roi(130, 110, 270, 270);
        //img = img(roi).clone();
        Mat mask = Mat(img.size(), CV_8UC1, { 255 });
        //Mat mask;
        //cv::cvtColor(imread(prefix + "mask.bmp"), mask, COLOR_BGR2GRAY);

        // padding to avoid rotating out
        int padding = 100;
        float imgMean = mean(img)[0];
        cv::Mat padded_img = cv::Mat(img.rows + 2 * padding, img.cols + 2 * padding, img.type(), cv::Scalar::all(imgMean));
        ...
        //Mat sobel_dx, sobel_dy, sobel_ag, edges, magnitude, smoothed;
        //GaussianBlur(padded_img, smoothed, Size(5, 5), 0, 0, BORDER_REPLICATE);
        ...
        //magnitude = sobel_dx + sobel_dy;
        //magnitude.convertTo(magnitude, CV_16U);
        //phase(sobel_dx, sobel_dy, sobel_ag, true);
        //Mat des
        //float thresh = threshold(magnitude, dest, 0, 255, THRESH_BINARY | THRESH_OTSU);
        ...
        shapes.angle_range = { 0, 90, 260, 360};
        shapes.angle_step = 1;
        ...
        auto infos = shape_based_matching::shapeInfo_producer::load_infos(prefix + "case2/test_info.yaml");

        Mat test_img = imread(prefix + "rotate_90.jpg");
        assert(!test_img.empty() && "check your img path");
        ...
        for (size_t i = 0; i < top5; i++) {
            ...
            for (int i = 0; i < templ[0].features.size(); i++) {
                auto feat = templ[0].features[i];
                cv::circle(img, { feat.x + match.x, feat.y + match.y }, 0.5, randColor, -1);
            }
...
void noise_test(string mode = "test") {
    line2Dup::Detector detector(80, { 1, 2 }, 15, 20);
    ...
    if (mode == "train") {
        ...
        Mat mask;
        cv::cvtColor(imread(prefix + "template.jpg"), img, COLOR_BGR2GRAY);
        //cv::cvtColor(imread(prefix + "case3/mask.bmp"), mask, COLOR_BGR2GRAY);
        mask = Mat(img.size(), CV_8UC1, { 255 });

        shape_based_matching::shapeInfo_producer shapes(img, mask);
        shapes.angle_range = { 0, 2 };
        shapes.angle_step = 1;
        shapes.produce_infos();
        std::vector<shape_based_matching::shapeInfo_producer::Info> infos_have_templ;
        string class_id = "test";
        ...
        /*Mat test_img = imread(prefix + "case3/1234.bmp");
        assert(!test_img.empty() && "check your img path");*/

        Mat test_img, mask;
        cv::cvtColor(imread(prefix + "noise3.jpg"), test_img, COLOR_BGR2GRAY);
        ...
        for (auto idx : idxs) {
            ...
            for (int i = 0; i < templ[0].features.size(); i++) {
                auto feat = templ[0].features[i];
                cv::circle(test_img, { feat.x + match.x, feat.y + match.y }, 0.5, randColor, -1);
            }
        ...
        Mat resized_down;
        //resize down
        resize(test_img, resized_down, Size(down_width, down_height), INTER_LINEAR);
        imshow("img", resized_down);
        waitKey(0);
